...
void ImuProcess::PublishOdometry(const state_ikfom &imu_state,
                                 double timestamp) {
  ...
  tf::Quaternion q_tf;
  transform.setOrigin(
      tf::Vector3(imu_state.pos.x(), imu_state.pos.y(), imu_state.pos.z()));
  q_tf.setW(q.w());
  q_tf.setX(q.x());
  q_tf.setY(q.y());
  q_tf.setZ(q.z());
...
void ImuProcess::UndistortPcl(
  ...
  /*    cout<<setprecision(18)<<"[ IMU Process ]: Process
     "<<meas.lidar->points.size()<<" lidar from "<<pcl_beg_time<<" to
     "<<pcl_end_time<<", " \
  ...
  // std::cout<< "pcl_out size in UndistortPcl " << pcl_out.size() << " "<<
  // pcl_out.empty() << std::endl;
}
  ...
  if (imu_need_init_) {
    ...
    if (init_iter_num > MAX_INI_COUNT) {
      ...
      ROS_INFO("IMU Initial Done: Gravity: %.4f %.4f %.4f %.4f; state.bias_g: "
               "%.4f %.4f %.4f; acc covarience: %.8f %.8f %.8f; gry "
               "covarience: %.8f %.8f %.8f",
